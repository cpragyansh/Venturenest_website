    ...
    <div className="min-h-screen bg-white font-jakarta">
      ...
      <section className="bg-white py-20">
        <div className="container mx-auto px-4">
          ...
          <p className="text-brand-dark text-center mb-16 font-lato">
            Factors that set CGC Jhanjeri apart from others in terms of the placements to ensure the success of its students.
          </p>

          <div className="grid md:grid-cols-4 gap-8 max-w-6xl mx-auto">
            {[
              { title: 'Leading Industry Employers', desc: 'Renowned industry entities, such as Amazon, Microsoft, IBM, Capgemini, & others are among 800+ key recruiters.', icon: '🏢' },
              { title: 'Training & Placement Cell', desc: 'CGC Jhanjeri runs a 360-degree Pre-Placement Training Program for overall student personality development under the CCPD Department.', icon: '📚' },
              { title: 'Industry-Linked Academic Partnerships', desc: 'Premier placements are assured through our robust industry-academia partnerships.', icon: '🤝' },
              { title: 'Industrial Internships', desc: 'CCG Jhanjeri arranges the internship program and Summer Training/ Final Semester Training with a vision to explore and gain industry exposure.', icon: '💼' },
            ].map((feature, idx) => (
              <div key={idx} className="text-center">
                <div className="bg-brand-red rounded-full w-32 h-32 flex items-center justify-center mx-auto mb-6 text-white text-5xl">
                  {feature.icon}
                </div>
                <h3 className="text-brand-red text-2xl font-bold font-lato mb-4">{feature.title}</h3>
                <p className="text-brand-dark font-lato">{feature.desc}</p>
              </div>
            ))}
          </div>
        </div>
